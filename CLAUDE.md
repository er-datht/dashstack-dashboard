...
9. `opsx:archive` — finalize; add the domain detail to [`openspec/SPECS-CATALOG.md`](openspec/SPECS-CATALOG.md) and, if the domain is new, one line to the **Existing Specs** index below
...
Per-domain history lives in [`openspec/SPECS-CATALOG.md`](openspec/SPECS-CATALOG.md), not in this file — it was extracted once it reached 55% of CLAUDE.md and was being loaded into every session regardless of relevance. When archiving, add the detail **there** and keep the **Existing Specs** index below at one line per domain. If a single catalogue entry outgrows its neighbours, split that domain into its own file rather than letting the catalogue sprawl. When spec files grow too large from accumulated deltas, split them by subdomain (e.g., `inbox/compose/spec.md`, `inbox/folders/spec.md`).

A constraint that must hold for *future* work belongs in **Common Gotchas** below, not in the catalogue — the catalogue is read on demand, so a hard rule buried there will be missed.
...
Domains below have archived OpenSpec changes. **Full per-domain detail — including the
non-obvious traps each change uncovered — is in [`openspec/SPECS-CATALOG.md`](openspec/SPECS-CATALOG.md).**
Read the relevant entry there before working in that area; this list is only a map of what exists.

- **Core Infrastructure** — build pipeline, TS config, design tokens, theme switching, routing, lazy loading, dual API clients, React Query, i18n config, unit testing setup
- **Dashboard** — dashboard composition, recharts widgets, DealDetailsTable
- **Calendar** — month/day/week views, add/edit/delete events, all-day row packing, popover clamping, viewport-bound modal
- **Modal Animation** — fade+scale show/hide for AddEventModal + ConfirmModal, deferred unmount, reduced motion
- **Todo** — per-card rows, starred/completed styling
- **Contact** — card grid, Add New Contact form, shared AddPersonForm
- **Team** — card grid, Add New Member form
- **Inbox** — two-panel layout, folders, compose, star/bin/spam, select-all
- **Invoice** — sender/recipient header, items table, Print/Send
- **Settings** — general settings form, logo upload, required-field asterisks
- **Manage Account** — user-scoped profile page, avatar data-URL persistence, auth-user-changed event
- **UI Elements** — charts gallery, per-theme palettes, chart shells, FilterByDropdown contrast fix
- **Tables Gallery** — tables showcase, 12 variant cards, `tables` i18n namespace
- **Shared Components** — TableCommon modifiers + keyboard rows, ColorDots, ConfirmModal promotion
