...
The form SHALL validate the following required fields on submit: First Name, Last Name, Email. Email SHALL also be validated for format (must contain @ and domain). When validation fails, invalid fields SHALL display a red border and an error message below the field. Error messages SHALL clear when the user modifies the respective field.
