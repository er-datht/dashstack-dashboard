...
Fluid not rigid
What it means	
Artifacts can be created in any order
Example	
A spike may produce specs before a proposal
Principle
Iterative not waterfall
What it means	
Revisit artifacts as understanding deepens
Example	
Proposal written before reading code → revise after
Principle
Easy not complex	
What it means	
Scale ceremony to the change
Example	
One-line fix → one-line proposal
Principle
Brownfield-first	
What it means	
Read existing specs before exploring code
Example	
Specs are source of truth; code is the detail
