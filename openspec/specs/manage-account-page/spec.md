# manage-account-page Specification

## Purpose
Defines the Manage Account page reachable from the TopNav UserMenu — an authenticated profile page where the user edits Display Name / Phone / Bio, uploads an avatar, and persists the patch back to the stored user record (with same-tab rebroadcast via `auth-user-changed`).
