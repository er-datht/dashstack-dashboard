# inbox-important Specification

## Purpose

Defines the per-message Important flag for the Inbox page. Each `EmailRecord` may carry an optional `isImportant` flag, persisted via the `useLocalStorage` hook under the storage key `"inbox-important-ids"`. Users can toggle the flag from three entry points — a per-row Bookmark button on eligible folders, a Bookmark button in the ChatHeader, and a bulk-action Bookmark button in the message list toolbar. A dedicated Important folder displays the filtered union of all flagged records (excluding binned, archived, and spammed), and the left sidebar shows a live Important count that mirrors that filtered set. The flag is preserved across moves to Bin, Archive, and Spam, and is restored when a record is restored from any of those folders. All user-visible strings (per-row tooltips, toolbar tooltips, ChatHeader tooltips, toast confirmations) ship in both `en/inbox.json` and `jp/inbox.json`.
