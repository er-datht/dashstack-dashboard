# loading-indicator-color Specification

## Purpose
Defines the colour contract for loading indicators: the single token that drives them, how it resolves per theme, which indicators are in scope, how translucent values are authored, and the contrast floor the accent must clear.
