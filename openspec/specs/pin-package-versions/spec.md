# pin-package-versions Specification

## Purpose
Requires all `package.json` version strings to be pinned exactly — no `^` or `~` range prefixes — so every install produces the same dependency tree.
