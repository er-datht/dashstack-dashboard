# calendar-view-state Specification

## Purpose

Defines the cross-view state and shared header behavior for the Calendar page: `viewMode` state with three values (day, week, month) controlling which view component renders, the `currentDate` state navigated by chevrons / Today / view-toggle / date-picker, the shared CalendarHeader component's prop contract, the CalendarEvent type and mock seed data, and the i18n translation keys used by the header.
## Requirements
