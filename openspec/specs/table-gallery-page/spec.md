# table-gallery-page Specification

## Purpose
Defines the Tables gallery page at `/table` — a showcase route presenting the project's `TableCommon` component across three sections (Basic Tables, Cell Content, States & Interaction). Each section demonstrates a distinct dimension of table styling and behavior via four small variant cards driven by hard-coded mock data, gated by a Filter By Tables dropdown. Mirrors the structure of the existing UI Elements charts gallery.
...
1. **Loading** — renders with `loading={true}` so the spinner overlay is visible. The Loading variant card SHALL render WITHOUT a subtitle: the centered spinner overlay is the variant's only label, so a top-left "Loading" subtitle conflicts visually with the centered overlay and is intentionally omitted. The variant card's bordered shell still renders so the variant remains a distinct card in the 2×2 grid.
...
- **THEN** four variant cards render — the Empty, Paginated, and Clickable Rows variants display their respective subtitles ("Empty", "Paginated", "Clickable Rows"); the Loading variant intentionally renders without a subtitle and is identified by its centered spinner overlay

#### Scenario: Loading variant has no subtitle
- **WHEN** the Loading variant card renders
- **THEN** no `<h3>` subtitle element renders inside its bordered shell; the card's only visible content is the `TableCommon` spinner overlay
